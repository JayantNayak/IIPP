...
def draw(canvas):
    for i in range(16):
        if exposed[i]:
            canvas.draw_polygon([[i*50, 0], [(i+1)*50, 0], [(i+1)*50, 100], [i*50, 100]], 1, "Blue", "White")
            canvas.draw_text(str(deck[i]), (i*50+11, 69), 55, "Blue")
        else:
            canvas.draw_polygon([[i*50, 0], [(i+1)*50, 0], [(i+1)*50, 100], [i*50, 100]], 1, "Blue", "Yellow")
    label.set_text("Turns Finished = " + str(nMoves))
